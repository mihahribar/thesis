...
namespace Thesis {
    ...
    // without a default constructor std::map know what to do
    Date::Date() {
        setDate(1970,1,1);
    }
    ...
    void Date::setDay(int d) {
        if (d == 0) {
            ...
            // etc
            day = daysInMonth(month, year) + d + 1;
    ...
    Weekday Date::getWeekday() {
        // Sakamoto, Lachman, Keith and Craver algorithm
        int d = day;
        int m = month;
        int y = year;
        return static_cast<Weekday>((d+=m<3?y--:y-2,23*m/9+d+4+y/4-y/100+y/400)%7);
    }
