...
\newglossaryentry{jit}{name=JIT, description={\eng{Just-in-time compilation}; tehnika, znana tudi pod imenom dinamično prevajanje, kjer se prevod ne zgodi pred zagonom programa, ampak ravno pred}}
...
%==============================
\chapter[Pregled metod medplaformnega razvoja]{Pregled metod \\ medplaformnega razvoja}
\label{chap:overview}
...
Java, ki jo srečamo na platformi Android, se od odprtokodne Jave kar precej razlikuje. Android ne uporablja Java virtualnega pogona (\eng{Java Virtual Machine}) ampak svoj pogon Dalvik, ki je prilagojen za uporabo na mobilnih napravh. Pred kratkim je Google napovedal nov virtualen pogon, Android Runtime (\gls{art}), ki bo v prihodnosti zamenjal Dalvik in vsebuje veliko izboljšav v hitrosti in stabilnosti aplikacij.
...
Razlika v hitrosti izvajanja med kodo napisano v jeziku C++ in Objective-C je zanemarljiva, saj je Objective-C za platformo iOS domoroden jezik (\eng{native programming language}). To lahko preverimo z implementacijo algoritma za generiranje Fibonaccijevega zaporedja v jeziku C++ in Objective-C (slika \ref{fig:ios-performance}).
...
V raziskavi Univerze na Dunaju\cite{android-cpp} so ugotovili, da je Java s pomočjo \gls{jit} prevajalnika zmožna na preprostih primerih (recimo generiranje Fibonaccijevega zaporedja) prehiteti C++ kodo. V nekoliko kompleksnejših primerih uporabe (recimo branje \gls{xml} dokumentov), kjer JIT prevajalnik ni bil zmožen pohitriti Java kodo, pa je bil C++ vseeno hitrejši.

Avtorji na koncu navedejo, da vključevanje C++ kode v Android aplikacije nebi smelo imeti velikega vpliva na hitrost, saj je bilo izvajanje kode v večini primerov primerljivo.

\subsection{C\# in C++}

Za večino primerov je hitrost izvajanje C\# kode primerljiva izvajanju C++ kode. Razlike se začnejo kazati pri procesorsko zahtevnejših algoritmih, kjer začne prevladovati C++ koda, predvsem zaradi boljše izrabe pomnilnika\cite{windows-phone-dev}. Zavedati se moramo, da pri prevelikem prehajanju iz domorodne kode v C\# kodo lahko pride do izničitve kakršnekoli prednosti C++ kode, saj v vmesnem delu pride do pretvarjanja podatkovnih tipov iz C++ sveta v C\# svet (in obratno).

Po vsem tem lahko pridemo do istega zaključka kot v Java primeru - vključitev C++ knjižnice v našo Windows Phone aplikacijo v večini primerov ne bo imelo prevelikega vpliva na hitrost.

\subsection{JavaScript in C++}

Na področju JavaScripta je bilo v zadnjih nekaj letih videti velik napredek. S pomočjo asm.js\cite{asmjs}, ki je podmnožica jezika JavaScript, je spletnim brskalnikom v letu 2014 uspelo doseči le 2-kratno upočasnitev v primerjavi z domorodno kodo. Ista raziskava iz leta 2011 je pokazala kar 12-kratno upočasnitev\cite{html5-gamedev}.

Neposredno pisanje v asm.js je dokaj nerealno, saj razvijalca sili v pisanje kode na način, ki jo najbrž ni vajen (glej primer \ref{code:asm.js}). Namesto tega je asm.js še posebej primeren kot destinacijski jezik za prevajanje med jeziki z ogrodjem kot je Emscripten. Koda prevedena iz jezika C++ bo zaradi tega v večini primerov hitrejša, kot tista, ki jo razvijalec sam napiše v jeziku JavaScript.
